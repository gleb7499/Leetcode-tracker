...
Stage 7 done (2026-09): product readiness — README rewritten as the single entry point (quick start, OTP-from-logs, Maildev/SMTP modes, backup, self-hosting, developer docs), license switched to MIT (2026, gleb7499), GitHub Actions CI (backend gradle build with Testcontainers, frontend npm ci/typecheck/lint/test/build). Verified: clean clone → `docker compose up --build` → full E2E.

Stage 8 done (2026-09): final polish. Backend: N+1 removed from task list/today queue and backup export (batched topic/review loads via `findByTaskIdIn` / `findByUserTaskIdIn` with EntityGraph); new edge-case tests (refresh with unknown token, consumed-OTP reuse); existing edge cases verified covered (expired/exhausted OTP, refresh rotation and logout revocation, idempotent duplicate task, empty review queue, malformed/wrong-format backup). Indexes verified against hot queries with EXPLAIN — `idx_user_tasks_next_review` and `idx_user_reviews_user_task` cover the review queue and stats aggregations, no new migration needed. Frontend: daily-progress ring re-syncs to the real queue size and is stored per user; home empty-state and load-error states added; add-task topic input accepts free-form topics via Enter. All screens re-verified against the compose stack, final screenshots in `docs/screenshots/final-*.png`.
